function [x,x0,fval] = lsqfit_ms(sq, acu, mean, std)
    ...
    path = 20;   %multistart path number
    opts = optimoptions(@lsqcurvefit, 'Algorithm','trust-region-reflective','TolFun',10^acu);  % TR needs gradient and GradOgj='on'
    problem = createOptimProblem('lsqcurvefit', 'objective', @sqfactor, ...
    'xdata', xdata, 'ydata', ydata, 'x0', x0, 'lb', lb, 'ub', ub, 'options', opts);
    ms = MultiStart('UseParallel', false, 'Display','iter','MaxTime',10000,'TolFun',10^acu,...
    'StartPointsToRun', 'bounds');
    [x,fval] = run(ms,problem, path);
end